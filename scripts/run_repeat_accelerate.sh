...
# Usage: ./scripts/run_repeat_accelerate.sh [START_IDX] [COUNT]
# Env overrides:
...
#   TOP_P (default: 1)
#   SKIP_SECS (default: 1)
#   ACCELERATE_PORT (default: BASE_PORT + 512 + idx)
...
for ((i=0; i<COUNT; i++)); do
  ...
  if [[ -n "${ACCELERATE_PORT:-}" ]]; then
    EDIT_PORT=$ACCELERATE_PORT
  else
    EDIT_PORT=$((BASE_PORT + 512 + (IDX % 256)))
  fi

  echo "[EDIT|accelerate] case_index=$IDX using GPUs $GPUS port=$EDIT_PORT"
