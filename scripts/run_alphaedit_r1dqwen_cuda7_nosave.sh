...
# 单卡 AlphaEdit（DeepSeek-R1D-Qwen-7B），使用 CUDA 7，就地编辑后直接生成（不保存/不重载）。
# 用法：
#   ./scripts/run_alphaedit_r1dqwen_cuda7_nosave.sh [START_IDX] [COUNT]
# 可覆盖环境变量：
#   GPU=7
#   HPARAMS=hparams/AlphaEdit/deepseek-r1d-qwen-7b.yaml
#   DATA=data/noncot.json
#   OUT_JSONL=output/alphaedit_r1dqwen_cuda7.jsonl
#   GEN_MODE=r1d
#   MAX_NEW_TOKENS=1024
...
OUT_JSONL=${OUT_JSONL:-"output/alphaedit_r1dqwen.jsonl"}
